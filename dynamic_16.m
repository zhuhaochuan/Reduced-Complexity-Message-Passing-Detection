...
function [out1,out2,out3,out4,out5,out6,out7,out8] = dynamic_16(K,J,Z,N0v,s,t,Es,E_guiyi)
    ...
    %这是对上次迭代结果的概率排序的确对每一个符号排序现在考虑增加条件判断
     for k_i=1:2*K%动态的决定要取几个
        ...
        for j_k=1:cs%逐个判断只要出现在范围内的就跳出当前循环执行下次循环否则会出现上面提到的问题
            if (pro(k_i,j_k)>0.5)&&(pro(k_i,j_k)<0.9)%大于0.5取1个
                index(k_i,1)=index_(k_i,4);
                index(k_i,2)=5;
                break;
