...
function [out1,out2,out3,out4,out5,out6,out7,out8] = dynamic_256(K,J,Z,N0v,s,t,Es,E_guiyi)
...
for t=2:s% 迭代的循环 
    %这是对上次迭代结果的概率排序的确对每一个符号排序现在考虑增加条件判断
     for k_i=1:2*K%动态的决定要取几个
        ...
        %这里的动态标准是 出现0.5-0.9之间的就取4个 出现0.9以上的就缩小到2个其余6个
        %其余情况都是取8个这样动态的选择会进一步降低计算的复杂度
        for j_k=1:cs%逐个判断只要出现在范围内的就跳出当前循环执行下次循环否则会出现上面提到的问题
            if (pro(k_i,j_k)>0.5)&&(pro(k_i,j_k)<0.9)%大于0.5取4个
                index(k_i,1:4)=index_(k_i,13:16);
                index(k_i,5:8)=17;
                break;
           
            elseif  pro(k_i,j_k)>=0.9%大于0.9取2个
