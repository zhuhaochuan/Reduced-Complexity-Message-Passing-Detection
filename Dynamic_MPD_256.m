...
K =32;%用户天线数
N =128;%基站天线数
t=4;%选择调制阶数，按照下面的M中的顺序
s = 13;%算法迭代的次数
SNRdBs =16:30;%信噪比SNR的范围
MAX_nobit=100;%最大误码数
...
fprintf('SNRdBs\t\t平均使用的概率点数\t比例\n');
fprintf('SNRdBs\t\t乘法数\t加法数\t原算法乘，加数\t比例\n');
fprintf('SNRdBs\t\tBER\n');%打印出每次信噪比下的误码率
...
for i_SNR = 1:length(SNRdBs)  %对于信噪比变化的大循环
   ...
   while(1)
     ...
     %输入数据并检测的循环
      for tx_time = 1 : (syn_per_ant * Num_pack/can_s) %每次进入8K个bit数据，这是传输次数的循环
        ...
        [L,pp,dd,bb,LL,in,num,dy]=dynamic_256(K,J,Z,N0v,s,t,1,E_guiyi);
        
         %动态选择的计数：
         num2(tx_time) = sum(sum(sum(dy)));
       ...
       %解调过程
       demod = modem.qamdemod('M',M,'OutputType','Bit','DecisionType','hard decision','SymbolOrder','gray','NoiseVariance',N0/2);
        ...
        receive_bit(can_s*(tx_time-1)*K+1:can_s*tx_time*K,1) =x_bit_;
         ...
         %使用的总共的概率点个数除以原始算法采用的概率点的个数：
         s_num1(count) = sum(num2);
         p_num1(count) = s_num1(count)/((syn_per_ant * Num_pack/can_s)*2*K*sqrt(M)*s);
  ...
  %误码率的计算
   ...
   s_num(i_SNR) = sum(s_num1)/count;
   p_num(i_SNR) = sum(s_num1)/(((syn_per_ant * Num_pack/can_s)*2*K*sqrt(M)*s)*count);
   
   
   %将平均每次使用的概率点数，以及比例输出：
   fprintf('%d\t%f\t%f\n',SNRdB,s_num(i_SNR)/(tx_time*s*2*K), p_num(i_SNR));
    
    
    %计算D-MPD计算复杂度：
    n_a(i_SNR) = s_num(i_SNR)/(tx_time*s*2*K);
    mut(i_SNR) = (2*n_a(i_SNR)+1)*(2*K-1)*2*K*s;
    add(i_SNR) = ((2*n_a(i_SNR)+1)*(2*K-1)-2)*2*K*s;
    
    %原始算法的复杂度计算和比较：
    mut_mpd = (2*sqrt(M)+1)*(2*K-1)*2*K*s;
    add_mpd = ((2*sqrt(M)+1)*(2*K-1)-2)*2*K*s;
    fprintf('%d\t%f\t%f\t%d\t%d\t%f\t%f\n ',SNRdB,mut(i_SNR),add(i_SNR),mut_mpd,add_mpd ,mut(i_SNR)/mut_mpd,add(i_SNR)/add_mpd);
...
toc  %计算算法执行时间
 ...
 %制作相应表格显示误比特率和信噪比的关系
 %semilogy(SNRdBs(1:length(BER)),BER,'c<-','LineWidth',2);
 % xlabel('Average SNR in dB');%横坐标
 %ylabel('Uncoded BER');%纵坐标
 % title('N=128,K=16,256QAM');   %标题
 %legend('D-MPD');%标注
 %grid on%加网格线